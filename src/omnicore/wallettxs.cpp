...
bool CheckInput(const CTxOut& txOut, int nHeight, CTxDestination& dest)
{
    ...
    if (!ExtractDestination(txOut.scriptPubKey, dest)) {
        PrintToLog("(%s): !ExtractDestination \n", __func__);
        return false;
    }
...
static int64_t GetEstimatedFeePerKb()
{
    ...
    // if (pwalletMain) {
    //     nFee = GetMinimumFee(*(pwalletMain),1000, coin_control, &feeCalc);
    // }
#endif
...
int64_t SelectCoins(interfaces::Wallet& iWallet, const std::string& fromAddress, CCoinControl& coinControl, int64_t additional)
{
     ...
     CAmount nMax = 1 * GetEstimatedFeePerKb(iWallet);
