...
int mastercore::GetEncodingClass(const CTransaction& tx, int nBlock)
{
    ...
    for (unsigned int n = 0; n < tx.vout.size(); ++n) {
        ...
        PrintToLog("(%s): outType == TX_PUBKEYHASH\n");
            CTxDestination dest;
        ...
        if (outType == TX_NULL_DATA) {
         ...
         PrintToLog("(%s): outType == TX_NULL_DATA\n", __func__);
        // if (true) {
            ...
            if (!scriptPushes.empty()) {
                PrintToLog("(%s): !scriptPushes.empty()\n", __func__);
    ...
    if (hasExodus || hasMoney) {
        PrintToLog("getEncodingClass:OMNI_CLASS_A\n");
        return OMNI_CLASS_A;
    }
...
static bool FillTxInputCache(const CTransaction& tx)
{
    ...
    for (std::vector<CTxIn>::const_iterator it = tx.vin.begin(); it != tx.vin.end(); ++it) {
        ...
        if (view.HaveCoin(txIn.prevout)){
             PrintToLog("(%s):view.HaveCoin(txIn.prevout)\n",__func__);
             ++nCacheHits;
             continue;
        }else{
            PrintToLog("(%s):#### else ####\n",__func__);
...
static int parseTransaction(bool bRPConly, const CTransaction& wtx, int nBlock, unsigned int idx, CMPTransaction& mp_tx, unsigned int nTime)
{
    ...
    if (omniClass != OMNI_CLASS_C)
    {
       ...
       for (unsigned int i = 0; i < wtx.vin.size(); ++i) {
           ...
           if (ExtractDestination(txOut.scriptPubKey, source)) { // extract the destination of the previous transaction's vout[n] and check it's allowed type
               std::string addressSource = EncodeDestination(source);
               inputs_sum_of_values[addressSource] += txOut.nValue;
           }
           else return -106;
      }
        ...
        if (omniClass == OMNI_CLASS_A) {
            ...
            if (!strDataAddress.empty()) { // Step 2, try to locate address with seqnum = DataAddressSeq+1 (also verify Step 1, we should now have a valid data packet)
                ...
                for (unsigned int n = 0; n < wtx.vout.size(); ++n) {
                    CTxDestination dest;
                    if (ExtractDestination(wtx.vout[n].scriptPubKey, dest)) {
                        if (EncodeDestination(dest) == ExodusAddress()) {
    ...
    if (omniClass == OMNI_CLASS_C) {
        ...
        PrintToLog("(%s):### CLASS B / CLASS C PARSING ###\n",__func__);
        ...
        PrintToLog("Ending reference identification\nFinal decision on reference identification is: %s\n", strReference);


        // ### CLASS C SPECIFIC PARSING ###
        if (omniClass == OMNI_CLASS_C) {
           PrintToLog("(%s):omniClass == OMNI_CLASS_C\n",__func__);
            ...
            for (unsigned int n = 0; n < wtx.vout.size(); ++n) {
                ...
                if (whichType == TX_NULL_DATA) {
                  PrintToLog("(%s):whichType == TX_NULL_DATA\n",__func__);
    ...
    if (omniClass == OMNI_CLASS_A && packet_size == 0) {
        PrintToLog("(%s):omniClass == OMNI_CLASS_A && packet_size == 0\n",__func__);
        return 1;
    }
...
int mastercore_init()
{
    ...
    // PrintToLog("Initializing Omni Core v%s [%s]\n", OmniCoreVersion(), Params().NetworkIDString());

    PrintToLog("\nInitializing Omni Core \n");
...
//
//     bool wrongDBVersion = (p_txlistdb->getDBVersion() != DB_VERSION); //DB_VERSION=6
...
//
    ...
    PrintToLog("Omni Core initialization completed\n");
    return 0;
}
...
int mastercore::WalletTxBuilder(const std::string& senderAddress, const std::string& receiverAddress, const std::string& redemptionAddress, int64_t referenceAmount, const std::vector<unsigned char>& payload, uint256& retTxid, std::string& retRawTx, bool commit, interfaces::Wallet* iWallet)
{
#ifdef ENABLE_WALLET
...
// Select the inputs
if (0 > SelectCoins(*(iWallet), senderAddress, coinControl, referenceAmount)) { return MP_INPUTS_INVALID; }
...
auto wtxNew = iWallet->createTransaction(vecRecipients, coinControl, true, nChangePosInOut, nFeeRet, strFailReason);
